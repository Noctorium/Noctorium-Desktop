...
class MpvOrphanTest {
    ...
    private fun playableFile(): Path? {
        app.spiceity.downloads.DownloadStore().all().firstOrNull()?.let { entry ->
            app.spiceity.downloads.DownloadStore().fileFor(entry)?.takeIf(Files::isRegularFile)?.let { return it }
        }
        ...
        return runCatching {
            ...
            val file = Files.createTempFile("spiceity-silence", ".wav")
