 ...
 *
 * This is the question behind "it works on yours and not on mine". A packaged Spiceity carries its own
...
class BackendLocatorTest {
    ...
    private fun resourcesWith(vararg executables: String): Path {
        val root = Files.createTempDirectory("spiceity-resources").also(made::add)
    ...
    fun `a resources directory without a bin folder is not mistaken for one`() {
        val root = Files.createTempDirectory("spiceity-empty").also(made::add)
        System.setProperty(property, root.toString())
