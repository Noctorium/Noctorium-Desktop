 ...
 * Run it deliberately:
 *
 *     ./gradlew :desktop:test --tests "*PlaybackToolInstallTest*" -Dspiceity.installTools=true
 */
class PlaybackToolInstallTest {
    ...
    fun `a machine with nothing on it ends up able to play music`() {
        if (!enabled) return
        val base = Files.createTempDirectory("spiceity-tools-test")
        try {
            ...
            assertTrue(state.ready, "everything installed and Spiceity still says it cannot play: $state")
        } finally {
