 ...
 *
 * ```
 * NOCTORIUM_LIVE_CHECK=1 ./gradlew test --tests 'app.noctorium.downloads.DownloadManagerLiveTest'
 * ```
 */
class DownloadManagerLiveTest {
    ...
    fun `a track goes from queued to downloaded, and is then playable from the disk`() = runBlocking {
        if (!asked) {
            println("MGR: skipped; set NOCTORIUM_LIVE_CHECK=1 to run it")
            return@runBlocking
        }
        val folder: Path = Files.createTempDirectory("noctorium-manager")
