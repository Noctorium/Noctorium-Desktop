 ...
 *
 * ```
 * NOCTORIUM_LIVE_CHECK=1 ./gradlew test --tests 'app.noctorium.downloads.LiveDownloadCheck'
 * ```
 */
class LiveDownloadCheck {
    ...
    fun `a real track downloads and mpv can read it`() = runBlocking {
        if (!asked) {
            println("DL: skipped; set NOCTORIUM_LIVE_CHECK=1 to run it")
            return@runBlocking
        }
        val folder = Files.createTempDirectory("noctorium-download")
        try {
