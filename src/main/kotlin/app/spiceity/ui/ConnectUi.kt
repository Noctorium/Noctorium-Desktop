...
/**
 * Spiceity Connect, from the desktop.
 *
 ...
 * whole thing is keyed on a secret only devices signed in to the same Spiceity can produce.
 */
@Composable
    ...
    Row {
        ...
        DropdownMenu(open, { open = false }, modifier = Modifier.widthIn(min = 260.dp)) {
            Column(Modifier.padding(horizontal = 14.dp, vertical = 8.dp)) {
                Text("Spiceity Connect", fontWeight = FontWeight.SemiBold, fontSize = 13.sp)
                Text(
            ...
            if (connect.devices.isEmpty()) {
                Text(
                    if (connect.available) {
                        "No other devices yet. Open Spiceity on your phone, on the same wifi, signed in to this account."
                    } else {
                        "Sign in to your Spiceity account to use Connect."
                    },
    ...
    Column(Modifier.fillMaxWidth()) {
        ...
        Text(
            if (connect.available) {
                ...
                "Sign in to your Spiceity account, above, and your devices will find each other."
            },
            color = MaterialTheme.colorScheme.onSurfaceVariant,
            fontSize = 11.sp,
        )
