 ...
 *
 * Deliberately not a thing that happens on its own. Spiceity looks once at launch and says so if there is
...
internal fun UpdatePanel(state: AppState) {
    ...
    Column(Modifier.verticalScroll(rememberScrollState())) {
        SettingsPanelCard {
            Text(
                if (available != null) "Spiceity ${available.version} is out" else "Spiceity is up to date",
                fontSize = 17.sp,
            ...
            if (available != null) {
                ...
                when {
                    ...
                    available.sha256 == null -> Text(
                        "This release did not publish a checksum, so Spiceity will not run its installer. " +
                            "The release page has the download.",
                        Modifier.widthIn(max = 520.dp),
                        color = MaterialTheme.colorScheme.onSurfaceVariant,
                        fontSize = 12.sp,
        ...
        if (available != null && updates.canInstall) {
            ...
            SettingsPanelCard {
                ...
                AccountFact("Spiceity closes so the installer can replace its files.")
