...
/**
 * Where the programs Spiceity plays music with are found, in the order they are trusted.
 *
 * The order is the point. A packaged Spiceity carries its own mpv and its own yt-dlp, and those are what
...
object BackendLocator {
     ...
     * A copy only appears in the application data folder because Spiceity deliberately fetched a newer
     ...
     */
    fun ytDlp(): Path? = locate("SPICEITY_YTDLP_PATH", if (isWindows) "yt-dlp.exe" else "yt-dlp", preferDownloaded = true)
    ...
    /** Whether this program is the copy that shipped with Spiceity. */
