...
/**
 * The programs Noctorium plays music with, and how they got here.
 *
 * This panel exists because an error message promised it did. "mpv is missing. Install it in Noctorium
 * Settings" was what a fresh install said the first time somebody pressed play, and Settings had no such
 * page -- so the one instruction Noctorium gave led nowhere. Mostly nobody should ever see this: what it
...
internal fun PlaybackToolsPanel() {
    ...
    Column(Modifier.verticalScroll(rememberScrollState())) {
        SettingsPanelCard {
            ...
            Text(
                "Noctorium uses two separate programs to play music. They are kept in Noctorium's own " +
        ...
        SettingsPanelCard {
            ...
            AccountFact("A copy you installed yourself is found and left alone — Noctorium never replaces it.")
...
private fun describe(tool: PlaybackTool, tools: PlaybackToolsState, windows: Boolean): String {
    ...
    return when (status.origin) {
        ...
        // Noctorium runs the copy it shipped with, and every install of one release runs the same one.
        ToolOrigin.BUNDLED -> "${tool.purpose} — built in"
        // Saying which copy is being used matters: somebody with their own mpv on PATH and a Noctorium
        // one in its folder should be able to see, without guessing, which of the two is playing.
        ToolOrigin.MANAGED -> "${tool.purpose} — installed by Noctorium"
