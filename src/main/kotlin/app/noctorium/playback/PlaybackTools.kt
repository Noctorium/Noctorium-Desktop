...
/**
 * The programs a desktop Noctorium shells out to, and what goes wrong when they are not there.
 *
 * A phone needs none of this: the extractor is compiled into the APK and the player is part of Android. On
 * a desktop both are separate programs, and until this file existed Noctorium simply assumed somebody had
 * installed them. A fresh install could not play a single track, and said so by telling the listener to
 * "install it in Noctorium Settings" -- where there was nothing to install it with.
 */
enum class PlaybackTool(
    val displayName: String,
    val purpose: String,
    /** Without this, Noctorium cannot play anything at all. */
...
enum class ToolOrigin {
    ...
    /** In Noctorium's own bin folder, which means Noctorium put it there and may replace it. */
    MANAGED,
...
data class PlaybackToolsState(
    ...
    /** Whether Noctorium can play a track right now. */
